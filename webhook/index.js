...
app.post('/webhook', async (req, res) => {
  ...
  const processConfig = { ...req.body, workerId }
  ...
  execa('node', ['processConfig.js', configFilePath], {
    all: true,
    detached: true
  }).all.pipe(fs.createWriteStream(`logs/pelias-update-log-${workerId}.txt`))
 ...
 */
app.get('/webhook/status/:id', async (req, res) => {
  const { id } = req.params
  ...
  const contents = await readFile(`logs/status-${id}.json`)
